...
public class OrderServiceImpl implements OrderService {
    ...
    public XcOrders saveXcOrders(String userId, AddOrderDto addOrderDto) {
        //幂等性处理，同一个选课记录（
        // 也就是数据库里的out_business_id，out_business_id是xc_choose_course表中的主键）只能有一个订单
    ...
    public void saveAliPayStatus(PayStatusDto payStatusDto) {
        ...
        XcPayRecord payRecord = getPayRecordByPayno(payNo);  //从数据库中查询这条支付记录
        if (payRecord == null) {
            XueChengPlusException.cast("支付记录找不到");
        }


        String status = payRecord.getStatus();//从数据库中查询的支付状态
        if ("601002".equals(status)) {  //如果已支付
            return;
        }
        ...
        if (trade_status.equals("TRADE_SUCCESS")) {  //支付宝返回TRADE_SUCCESS即为支付成功
